...
<script>
    ...
    $(function () {
        $('#tab').datagrid({
            ...
            fit:true,
            fitColumns:true,
            url:'${pageContext.request.contextPath}/puser/Paging.do',
            columns:[[
                {field:'id',title:'编号',width:25},
                {field:'name',title:'真实姓名',width:25},
                {field:'faname',title:'法号',width:25},
        ...
        $('#sel').combobox({
           valueField:'id',
           textField:'upname',
           url:'${pageContext.request.contextPath}/puser/findupname.do',
           width:100,
           select:0,
        });
    ...
    function del(id){
     $.messager.confirm('确认','您确定想要删除吗',function(f){
      if(f){
      $.ajax({
      url:'${pageContext.request.contextPath}/puser/delete.do',
    ...
    function update(id){
        $('#update').dialog({
           ...
           handler:function(){
            $('#ff').form({
             url:"${pageContext.request.contextPath}/puser/update.do?id="+id,
             onSubmit(){
